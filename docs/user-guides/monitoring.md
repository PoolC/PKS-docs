...
- [부록](#부록)
  ...
  - [애플리케이션 로그와 Ingress 로그의 차이](#애플리케이션-로그와-ingress-로그의-차이)
...
- **Ingress 로그 (Logs / Ingress-Nginx)**: 클러스터 외부에서 내부의 서비스로 들어오는 모든 HTTP 요청에 대한 기록입니다. 어떤 사용자가 언제, 어떤 주소로, 어떤 메소드(GET, POST 등)를 사용해 접속했는지 등의 정보를 담고 있어 트래픽 분석이나 접근 제어에 유용합니다.